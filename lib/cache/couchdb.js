...
CouchDB.prototype = {
    ...
    connect: function() {
        var options = _.pick(this.config, 'cache', 'secure', 'forceSave');

        //The auth default is only there to provide an example, so remove it if the username or password are empty
        ...
        this.client = new (cradle.Connection)(this.config.host, this.config.port, options);
