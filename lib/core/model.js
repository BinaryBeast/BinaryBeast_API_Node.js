...
/**
 * API response cache TTL getter / setter
 *
 * Sets the ttl for future calls to the given value, otherwise returns the current value of not given one
 *
 * Set to 0 or false to disable cache
 *
 * @param {number|boolean} ttl
 * @return {number}
 */
Model.prototype.ttl = function(ttl) {
 ...
 *
 * @param {function|{callback: function, [context]: object, [arguments]: object}} options
 */
Model.prototype.fetch = function(options) {
    ...
    //No id to load
        ...
        utils.defer(options.callback, options.context, false, {message: 'Cannot fetch Model data without an ID', model: this});
    }

    //Already loaded
    else if(this.loaded) {
        utils.defer(options.callback, options.context, this);
    }

    var args = {};
    args[ this._config.id_key || 'id' ] = this.id;
    ...
    this._request('read', {
        arguments:  args,
        callback:   utils.create_callback(this._fetch_response, this, options.callback, options.context),
        context:    this
    });
};
 ...
 * @private
 */
Model.prototype._fetch_response = function(callback, context, data, error) {
    this.loading = false;
 ...
 *
 * @param {string} type
 * @param {{callback: function, [context]: object, [arguments]: object, [ttl]: number, [service]: string}} opts
 * @private
 */
Model.prototype._request = function(type, opts) {
    ...
    //Call the API
    this.bb.api.fetch(options.service, options.arguments, options.ttl, utils.create_callback(
        this._handle_response, this,
        type, options
    ));
};
/**
 ...
 *
 * @param {string} type
 * @param {{type: string, callback: function, context: object}} options
 * @param {Object} response
 * @param [error]
 * @private
 */
Model.prototype._handle_response = function(type, options, response, error) {

    if(response) {
        if(response.result == 200) {
            //Fetch the response key
            var key = this._response_key( type ) || 'data';
            ...
            //Found the data!
            if(data) {
                utils.defer(options.callback, options.context, data, null, options);
                return;
            }
        }
    }
    ...
    utils.defer(
        options.callback, options.context, false, {
            error: 'Invalid API Response', response: response, request_error: error
        }, options
    );
...
Model.prototype.set = function(data, value, silent) {
    ...
    //Import a single key => value pair
        ...
        if( typeof this[data] != 'undefined' ) {
            ...
            if(data == 'name' && this._config.name_key && this._config.name_key != 'name') {
                return this.set(this._config.name_key, value);
            }
            ...
            if(this[data] != value) {
                ...
                //Also set the name property if appropriate
                if( data == this._config.name_key && data != 'name' ) {
                    this['name'] = value;
                }
            }
